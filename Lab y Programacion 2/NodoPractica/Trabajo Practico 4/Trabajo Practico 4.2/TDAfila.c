...
{
    fila->cabeza = iniciaNodoDoble();
    fila->cola = iniciaNodoDoble();
}

void mostrarFila(stFila * fila)
{
    mostrarListaNodoDoble(fila->cabeza);
}

void agregarFila(stFila * fila, char nombre[],int edad)
{
    nodoDoble * nuevo = crearNodoDoble(nombre,edad);
    fila->cola = agregarAlFinalNodoDoble(fila->cola,nuevo);
    if(fila->cabeza == NULL)
    {
        fila->cabeza = fila->cola;
    }
    fila->cola = nuevo;
}
...
stPersona extraerDato(stFila * fila)
{
    ...
    if(fila->cabeza != NULL)
    {
        primero = verPrimero(fila->cabeza);
        fila->cabeza = borrarNodoDobleBuscado(fila->cabeza,primero.edad);
        if(fila->cabeza != NULL)
        {
            fila->cola = iniciaNodoDoble();
        }
...
{
    printf("%s%c%c FILA INICIO %c%c%s\n",VERDE,218,180,195,191,NORMAL);
    while(lista != NULL)
    {
        printfDato(lista);
...
nodoDoble * agregarNodoDobleOrden(nodoDoble * lista, nodoDoble * nuevoNodo)
{
    ...
    {
        if (nuevoNodo->humano.edad < lista->humano.edad)
        {
